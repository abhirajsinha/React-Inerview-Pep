...
function App() {
  return (
    <div>
        ...
        <ul>
          {products
            ...
            .map((prod) => {
              return {
                ...prod,
                discountedPrice: prod.price * (10 / 100),
              };
            })
            .map((prod) => (
              <li key={prod.id}>
                <strong>{prod.name}</strong> - Price: ${prod.price} - Discounted
                Pirce {prod.price - prod.discountedPrice} - {prod.category}
              </li>
            ))}
        </ul>
      </div>
      ...
      {names
        .filter((name, index) => {
          // index of returns the first occurence index of a val in arr
